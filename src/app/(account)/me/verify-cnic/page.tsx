...
const SLOTS: { id: FileSlot; label: string; hint: string }[] = [
...
export default function VerifyCnicPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!files.front || !files.back || !files.selfie) {
      setError("Please select all three images.");
      return;
    }
    setError("");
    setUploading(true);
    try {
      for (const slot of ["front", "back", "selfie"] as FileSlot[]) {
        const file = files[slot]!;
        // 1. Get presigned PUT URL
        const presignRes = await fetch("/api/upload/presigned", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ fileType: slot, contentType: file.type }),
        });
      ...
      // 3. Mark as PENDING
      const pendingRes = await fetch("/api/account/request-verification", { method: "POST" });
      ...
      <main className="min-h-screen bg-gray-50 px-4 py-8 flex items-center justify-center">
        <div className="bg-white rounded-2xl border border-gray-200 p-8 max-w-md w-full text-center">
          ...
          <p className="text-sm text-gray-500 mb-6">
            Your documents are under review. You will be notified once verified (usually within 24 hours).
          </p>
          <p className="text-xs text-amber-600 bg-amber-50 rounded-lg px-3 py-2">
            ⚠️ This is a mock verification process. Production systems require real NADRA integration. See compliance notes in README.
          </p>
  ...
  return (
    <main className="min-h-screen bg-gray-50 px-4 py-8">
      <div className="max-w-lg mx-auto">
        ...
        <p className="text-sm text-gray-500 mb-6">
          Upload your CNIC images and a selfie. Images are encrypted and only accessible to admins for review.
        </p>
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-6 text-xs text-amber-700">
          ⚠️ <strong>Mock verification:</strong> This stores images locally for admin review. Not connected to NADRA. See README compliance note.
        </div>
