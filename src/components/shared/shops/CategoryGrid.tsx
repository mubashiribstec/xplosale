  ...
  return (
    <div style={{ fontFamily: "var(--body)" }}>
      ...
      <div style={{
        ...
        {featured.map((cat) => (
          <Card key={cat.slug} cat={cat} count={counts[cat.label] ?? 0} large />
        ))}
      </div>
      ...
      <div style={{
        ...
        {rest.map((cat) => (
          <Card key={cat.slug} cat={cat} count={counts[cat.label] ?? 0} />
        ))}
        <Card cat={other} count={counts[other.label] ?? 0} />
      </div>
    </div>
  );
}
...
function Card({ cat, count, large = false }: { cat: (typeof CATEGORIES)[0]; count: number; large?: boolean }) {
  return (
    <Link
