...
export function ShopApplicationQueue() {
  const [apps, setApps] = useState<ShopApp[]>([]);
  ...
  const fetchQueue = useCallback(async () => {
    const res = await fetch("/api/admin/shop-applications");
    if (res.ok) {
      const { data } = await res.json() as { data: ShopApp[] };
      setApps(data);
    }
    setLoading(false);
  }, []);
  ...
  async function act(action: "approve" | "reject") {
    ...
    setActing(true);
    setMsg("");
    const res = await fetch(`/api/admin/shop-applications/${selected.userId}`, {
      method: "PATCH",
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div className="md:col-span-1 bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
        {apps.length === 0 ? (
          <p className="p-4 text-sm text-gray-400">No pending shop applications.</p>
        ) : (
          apps.map((a) => (
            <button
              key={a.id}
              onClick={() => { setSelected(a); setReason(""); setMsg(""); }}
              className={`w-full text-left px-4 py-3 hover:bg-gray-50 transition-colors ${selected?.id === a.id ? "bg-blue-50" : ""}`}
            >
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Storefront details verified"
